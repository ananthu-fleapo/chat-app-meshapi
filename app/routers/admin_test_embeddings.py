"""
Test router — POST /test/embeddings
...
async def _upsert_model_price(
    ...
    - model_prices: INSERT or UPDATE for (model_id, provider).
      supports_embeddings_api is always set to True.
    """
    ...
    if price_row is None:
        # New row — insert with embeddings capability enabled
        await db.execute(
        ...
        # Existing row — enable embeddings API flag only
        price_row.supports_embeddings_api = True
        logger.info("test_model_price_updated", model_id=model_id, provider=provider)


@router.post("/embeddings", response_model=list[ModelResult])
async def test_embeddings(
    body: TestEmbeddingsRequest,
    ...
    provider_model_id. On pass the model is upserted into `models` and
    `model_prices` with supports_embeddings_api=True.
    Only available in dev (ENV=dev).

    Example:
        curl -X POST http://localhost:8000/test/embeddings \\
          -H "Content-Type: application/json" \\
          -d '{
            "models": [{"model_id": "openai/text-embedding-3-small", "provider_model_id": "text-embedding-3-small", "prompt_usd_per_1k": "0.00002", "completion_usd_per_1k": "0.0"}],
            "input": "Say hi",
            "provider": "openai"
          }'
    """
    ...
    async def probe(entry: ModelEntry) -> ModelResult:
        req = EmbeddingsRequest(
            model=entry.provider_model_id,
            input=body.input,
            dimensions=body.dimensions,
            encoding_format=body.encoding_format,
        )
        try:
            await adapter.embeddings(req, api_key=api_key)
